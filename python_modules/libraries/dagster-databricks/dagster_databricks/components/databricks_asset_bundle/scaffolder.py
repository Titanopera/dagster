...
class DatabricksAssetBundleScaffolder(Scaffolder[DatabricksAssetBundleScaffoldParams]):
    ...
    def scaffold(self, request: ScaffoldRequest[DatabricksAssetBundleScaffoldParams]) -> None:
        ...
        rel_databricks_configs_path = os.path.relpath(
            request.params.databricks_configs_path, start=project_root
        )
        databricks_configs_path_str = f"{project_root_tmpl}/{rel_databricks_configs_path}"

        custom_configs_path_str = None
        if request.params.custom_configs_path:
            rel_custom_configs_path = os.path.relpath(
                request.params.custom_configs_path, start=project_root
            )
        ...
        scaffold_component(
            request,
            {
                "databricks_configs": databricks_configs_path_str,
